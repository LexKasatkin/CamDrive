...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    String urlAuth="http://95.170.177.173/mobile/api_native/login/";
    String session;
    String identity;
...
//            urlAuth=urlAuth+mPassword;
            ...
            dataAuthentication.saveText(sPref,session,identity, remember_code,mEmail,mPassword);
            urlAuth=urlAuth+"?username=";
            urlAuth=urlAuth+mEmail;
            urlAuth=urlAuth+"&password=";
            urlAuth=urlAuth+mPassword;
            HTTPrequest httPrequest=new HTTPrequest(dataAuthentication, urlAuth, sPref);
            String json_string=httPrequest.getJSONString();
            return json_string;
        }
            ...
            if (success != null) {
                try {
                    ...
                    if(jsonObject.has("status")) {
                        ...
                        if (status.equals("1")) {
                            Toast toast = Toast.makeText(LoginActivity.this, "Успешно", Toast.LENGTH_SHORT);
                            toast.show();
                            Intent intent=new Intent(LoginActivity.this, CamerasActivity.class);
                            startActivity(intent);
                        } else if (status.equals("0")) {
                            if (jsonObject.has("message")) {
                                Toast toast = Toast.makeText(LoginActivity.this, "Введите ваш логин или пароль снова", Toast.LENGTH_SHORT);
                                toast.show();
                            }
                        }
                    }
